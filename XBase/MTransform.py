...
class MTransform(MNodes.MNode):
    ...
    def get_vector_to(self, other):
        if isinstance(other, str):
            other = MTransform(other)
        vec = MVector(other.world_pos) - MVector(self.world_pos)
        return vec
...
class MTransformList(object):
    ...
    def __init_node_list(self):
        lst_type = mb.get_list_types(self.node_names)
        if len(lst_type) != 1:
            raise ValueError(f'Input joint list:{self.nodes} has more than one type:{lst_type}')
        if lst_type[0] == str:
            self.nodes = [MJoint(name) for name in self.node_names]
        elif lst_type[0] == self.MEMBER_TYPE:
    ...
    def create(cls, names: list):
        ...
        if not exist:
            for name in names:
                joint = cls.MEMBER_TYPE.create(name)
                nodes_created.append(joint)
